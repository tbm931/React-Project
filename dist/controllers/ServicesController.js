...
export const getServicesToClient = async (req, res) => {
    try {
        const services = await getServices(req);
        console.log(services);
        serviceLogger.info("Services retrieved successfully for business: " + req.business.businessName);
...
export const getServices = async (req) => {
    try {
        ...
        const services = servicesModel.map(service => service.get({ plain: true }));
        return services;
    }
    catch (err) {
        return null;

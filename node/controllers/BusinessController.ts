...
import { BusinessResponse, Business, businessDetails } from '../Types.js';
...
export const getBusinessSignIn: (userName: string, userId: number) => Promise<BusinessResponse> = async (userName: string, userId: number) => {
    ...
    try {
        ...
        if (businessMode != null) {
            const findBusiness: Business = businessMode.toJSON();
...
export const getBusinessesDetails = async (req: Request, res: Response) => {
    try {
        const businessMode = await businessModel.findAll();
        if (businessMode != null) {
            const businesses: Business[] = businessMode.map(business => business.toJSON());
            const bussinessesDetails: businessDetails[] = businesses.map(business => ({
                id: business.id,
